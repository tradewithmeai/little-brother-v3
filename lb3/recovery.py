...
class RecoveryReport:
    ...
    def summary_line(self) -> str:
        ...
        return (
            f"Recovery sweep: {self.temp_files_recovered}/{self.temp_files_found} "
            f"temp files recovered, {self.total_lines_salvaged} lines salvaged"
        )
...
def salvage_plain_ndjson(temp_path: Path) -> SalvageStats:
    ...
    try:
        ...
        if not valid_lines:
            logger.warning(f"Truncated file with no complete lines: {temp_path.name}")
            return SalvageStats(
        ...
        logger.info(
            f"Successfully salvaged {temp_path.name} -> {recovered_path.name} ({len(valid_lines)} lines)"
        )
...
def salvage_gzipped_ndjson(temp_path: Path) -> SalvageStats:
    ...
    try:
        ...
        if not valid_lines:
            ...
            # Log as warning since truncated files are expected
            logger.warning(f"Truncated file with no complete lines: {temp_path.name}")
...
def recover_all_temp_files(
    spool_base_dir: Path, monitors: Optional[list[str]] = None
) -> RecoveryReport:
    ...
    if monitors:
        monitor_dirs = [
            spool_base_dir / monitor
            for monitor in monitors
            if (spool_base_dir / monitor).exists()
        ]
    else:
        monitor_dirs = [
            d
            for d in spool_base_dir.iterdir()
    ...
    for monitor_dir in monitor_dirs:
        ...
        if monitor_stats:
            ...
            len(monitor_stats) - recovered
            lines = sum(s.lines_salvaged for s in monitor_stats)
            logger.info(
                f"Monitor {monitor_name}: recovered {recovered}/{len(monitor_stats)} temp files, {lines} lines salvaged"
            )
