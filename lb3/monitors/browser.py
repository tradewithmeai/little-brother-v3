...
class BrowserMonitor(MonitorBase):
    ...
    def __init__(self, 
        ...
        self._known_browser_exes = {
            ...
            'opera.exe', 'vivaldi.exe', 'safari.exe', 'iexplore.exe'
        }
    ...
    def _try_load_cdp_plugin(self) -> None:
        ...
        try:
            ...
            if (config.browser.integration.chrome_remote_debug_port > 0 and
                "browser_cdp" in config.plugins.enabled):
                ...
                if self._cdp_plugin.is_available():
                    ...
                    logger.info("CDP plugin loaded but not available (Chrome not running with debug port)")
                    self._cdp_plugin = None
            else:
    ...
    def _set_event_window_title_hash(self, event_data: Dict[str, Any], title: str) -> None:
