...
class MonitorEvent:
    ...
    @classmethod
    def create(
        cls, monitor_name: str, event_type: str, data: dict[str, Any]
    ) -> "MonitorEvent":
...
class BaseMonitor(ABC):
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run_wrapper, name=f"Monitor-{self.name}", daemon=True
        )
        self._thread.start()
    ...
    def stop(self) -> None:
        ...
        if self._thread and self._thread.is_alive():
            ...
            if self._thread.is_alive():
                self.logger.warning(
                    f"Monitor {self.name} thread did not stop gracefully"
                )
...
class MonitorBase(ABC):
    ...
    def __init__(self, dry_run: bool = False, scheduler: Optional[Scheduler] = None):
        ...
        Args:
            ...
            scheduler: Optional scheduler for deterministic timing
                (defaults to production scheduler)
        """
        ...
        if hasattr(config, "batch") and hasattr(config.batch, "flush_thresholds"):
            ...
            config_str = getattr(
                config.batch.flush_thresholds, f"{self.name}_events", "100 or 5.0s"
            )
    ...
    def start(self) -> None:
        ...
        # Start monitor thread
        self._thread = threading.Thread(
            target=self._run_loop, name=f"Monitor-{self.name}", daemon=True
        )
        self._thread.start()
    ...
    def start_inline_for_tests(self) -> None:
        ...
        self.logger.info(
            f"Started monitor {self.name} in inline mode (session: {self._session_id})"
        )
    ...
    def stop(self) -> None:
        ...
        if self._inline_mode:
            try:
                ...
                self.logger.error(
                    f"Error stopping monitor {self.name}: {e}", exc_info=True
                )
    ...
    def _validate_event(self, event_dict: dict[str, Any]) -> None:
        ...
        if "monitor" in event_dict and event_dict["monitor"] != self.name:
            raise ValueError(
                f"Event monitor '{event_dict['monitor']}' doesn't match monitor name '{self.name}'"
            )
    ...
    def _schedule_flush(self) -> None:
        ...
        self._flush_handle = self.scheduler.call_later(
            self.batch_config.max_time_s, flush_callback
        )
    ...
    def check_time_flush_inline(self) -> bool:
        ...
        with self._batch_lock:
            ...
            if (
                self._batch
                and current_time - self._last_flush_time >= self.batch_config.max_time_s
            ):
                self._flush_batch()
                return True
        return False
    ...
    def _print_events(self, events: list[DBEvent]) -> None:
        ...
        for event in events:
            ...
            print(
                f"[{self.name}] {event.action} at {event.ts_utc}: "
                f"{json.dumps(event_dict, indent=2)}"
            )
