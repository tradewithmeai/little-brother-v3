...
class ActiveWindowMonitor(MonitorBase):
    ...
    def _setup_win32_hook(self) -> None:
        ...
        try:
            ...
            if not self._hook_handle:
                self.logger.warning(
                    "Failed to set Win32 event hook, using polling only"
                )
                return
    ...
    def _cleanup_win32_hook(self) -> None:
        ...
        try:
            # Set stopping flag first to prevent late callbacks
            self._stopping = True

            # Give any pending callbacks a moment to complete
            import time
    ...
    def _run_loop(self) -> None:
        ...
        try:
            ...
            while not self.should_stop():
                ...
                with self._hook_lock:
                    self._pending_changes.copy()
                    self._pending_changes.clear()

                # Always check to confirm current state
                self._check_active_window()
            ...
            try:
                ...
                self.logger.error(
                    f"Error stopping active window monitor: {e}", exc_info=True
                )
    ...
    def _has_window_changed(self, current: WindowInfo) -> bool:
        ...
        return (
            current.hwnd != last.hwnd
            or current.title_hash != last.title_hash
            or current.exe_path_hash != last.exe_path_hash
            or current.pid != last.pid
        )
    ...
    def _get_or_create_app_id(self, exe_path_hash: Optional[str]) -> str:
        ...
        with db._get_connection() as conn:
            cursor = conn.execute(
                "SELECT id FROM apps WHERE exe_path_hash = ?", (exe_path_hash,)
            )
            result = cursor.fetchall()

        app_id = result[0][0] if result else new_id()

        # Cache the result
        self._app_cache[exe_path_hash] = app_id
        return app_id

    def _get_or_create_window_id(
        self, exe_path_hash: Optional[str], title_hash: str
    ) -> str:
        ...
        with db._get_connection() as conn:
            cursor = conn.execute(
                "SELECT id FROM windows WHERE app_id = ? AND title_hash = ?",
                (app_id, title_hash),
            )
            result = cursor.fetchall()

        window_id = result[0][0] if result else new_id()
            ...
            with db._get_connection() as conn:
                ...
                if window_info.exe_path_hash:
                    conn.execute(
                        """
                        INSERT INTO apps (
                            id, exe_name, exe_path_hash, first_seen_utc, last_seen_utc
                        )
                        VALUES (?, ?, ?, ?, ?)
    ...
    def _emit_window_change_event(self, window_info: WindowInfo) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"Emitted window change event for: {window_info.exe_name or 'unknown'}"
            )
