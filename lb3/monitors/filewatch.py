...
class FileWatchMonitor(MonitorBase):
    ...
    def _create_event_handler(self):
        ...
        class FileWatchEventHandler(self._FileSystemEventHandler):
            ...
            def on_moved(self, event):
                if not event.is_directory:
                    # Handle as renamed
                    self.monitor._on_file_event("renamed", event.dest_path, event.src_path)
    ...
    def _on_file_event(self, action: str, file_path: str, moved_from_path: str = None) -> None:
    ...
    def _get_or_create_file_record(self, path_hash: str, ext: str) -> str:
        ...
        try:
            ...
            cursor.execute(
                "SELECT id FROM files WHERE path_hash = ?",
                (path_hash,)
            )
            ...
            if result:
                ...
                cursor.execute(
                    "UPDATE files SET last_seen_utc = ? WHERE id = ?",
                    (now_utc, file_id)
                )
                ...
                cursor.execute(
                    ...
                    (file_id, path_hash, ext, now_utc, now_utc)
                )
                db._connection.commit()
                return file_id
                
        except Exception as e:
            logger.error(f"Database error managing file record: {e}")
