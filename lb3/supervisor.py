            ...
            try:
                ...
                def console_handler(ctrl_type):
                    if ctrl_type in (0, 2):  # CTRL_C_EVENT, CTRL_CLOSE_EVENT
                        logger.info(f"Console control event {ctrl_type}, shutting down...")
                        if not self.verbose:
                            print("\nShutting down gracefully...")
                        self.request_shutdown()
                        return True
            ...
            if status['monitor'] and hasattr(status['monitor'], '_thread'):
                ...
                if thread and thread.is_alive():
                    ...
                    if remaining_time > 0:
                        ...
                        if thread.is_alive():
                            logger.warning(f"{monitor_name} thread did not stop within timeout")
                    else:
                        logger.warning(f"Timeout exceeded while waiting for {monitor_name}")
    ...
    def wait_until_shutdown(self, check_interval: float = 0.5):
        ...
        while not self._shutdown_requested and self._running:
            ...
            if self.dry_run:
                ...
                if (heartbeat_status and heartbeat_status['started'] and 
                    heartbeat_status['monitor'] and hasattr(heartbeat_status['monitor'], '_running')):
                    # If heartbeat monitor has stopped, consider this natural completion
                    if not heartbeat_status['monitor']._running:
                        logger.info("Heartbeat monitor finished, shutting down naturally")
                        break
            else:
                ...
                for status in self._monitor_status.values():
                    if status['started'] and status['monitor']:
                        if hasattr(status['monitor'], '_running') and status['monitor']._running:
                            any_running = True
                            break
...
def create_standard_supervisor(dry_run: bool = False, verbose: bool = False, 
                              duration: int = 0) -> MonitorSupervisor:
