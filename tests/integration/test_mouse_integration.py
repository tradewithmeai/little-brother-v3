...
class TestMouseIntegration:
    ...
    def test_burst_mouse_activity_size_flush(self, manual_scheduler):
        ...
        batch_config = BatchConfig(
            max_size=32, max_time_s=10.0
        )  # Size-based flush at 32
    ...
    def test_comprehensive_stats_accuracy(self, manual_scheduler):
        ...
        monitor = MouseMonitor(
            dry_run=False, event_source=fake_source, scheduler=manual_scheduler
        )
    ...
    def test_end_to_end_mouse_simulation(self, fake_clock, manual_scheduler):
        ...
        total_recorded_moves = sum(
            json.loads(e.attrs_json)["moves"] for e in collected_events
        )
        total_recorded_distance = sum(
            json.loads(e.attrs_json)["distance_px"] for e in collected_events
        )
        ...
        total_recorded_scrolls = sum(
            json.loads(e.attrs_json)["scroll"] for e in collected_events
        )
