...
class TestKeyboardIntegration:
    ...
    def test_simulated_typing_events(self, mock_config, fake_clock, manual_scheduler):
        ...
        assert len(keyboard_events) >= 1, f"Expected at least 1 keyboard event, got {len(keyboard_events)}"
    ...
    def test_burst_detection_integration(self, mock_config, fake_clock, manual_scheduler):
    ...
    def test_batch_flushing_time_based(self, mock_config, fake_clock, manual_scheduler):
        ...
        executed_tasks = manual_scheduler.advance(0.6)
    ...
    def test_performance_under_load(self, mock_config, manual_scheduler):
        ...
        for i in range(1000):  # 1000 key events
            fake_source.simulate_keydown()
            fake_source.simulate_keyup()
        ...
        assert duration < 1.0, f"Keyboard monitor too slow: {duration:.2f}s for 1000 events"
    ...
    def test_end_to_end_typing_simulation(self, mock_config, fake_clock, manual_scheduler):
        ...
        total_events_processed = sum(json.loads(e.attrs_json)["keydown"] for e in keyboard_events)
