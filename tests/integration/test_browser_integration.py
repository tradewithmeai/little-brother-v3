...
class TestBrowserIntegration:
    ...
    def test_fallback_mode_browser_switching(self, fake_clock, manual_scheduler):
        ...
        window_sequence = [
            {'exe_name': 'chrome.exe', 'window_title': 'Google Search - Google Chrome', 'pid': 1001},
            {'exe_name': 'firefox.exe', 'window_title': 'Mozilla Firefox Start Page', 'pid': 1002},
            {'exe_name': 'chrome.exe', 'window_title': 'GitHub - Google Chrome', 'pid': 1001},
    ...
    def test_fallback_mode_non_browser_filtering(self, manual_scheduler):
        ...
        window_sequence = [
            {'exe_name': 'notepad.exe', 'window_title': 'Untitled - Notepad', 'pid': 2001},
    ...
    def test_fallback_mode_different_window_states(self, fake_clock, manual_scheduler):
        ...
        window_states = [
            ...
            {'exe_name': 'firefox.exe', 'window_title': 'Page 1 - Firefox', 'pid': 1002},
        ]
        
        with patch('lb3.monitors.base.publish_event', side_effect=collect_event):
            monitor.start()
            
            for i, window_state in enumerate(window_states):
                with patch.object(monitor, '_get_active_window_info', return_value=window_state):
                    monitor.run_monitor_cycle()
    ...
    def test_mixed_browser_types_comprehensive(self, fake_clock, manual_scheduler):
        ...
        browser_scenarios = [
            # Chrome variants
            {'exe_name': 'chrome.exe', 'window_title': 'Google Search - Google Chrome', 'pid': 1001},
            {'exe_name': 'chrome.exe', 'window_title': 'YouTube - Google Chrome', 'pid': 1001},
            
            # Firefox
            {'exe_name': 'firefox.exe', 'window_title': 'Mozilla Firefox', 'pid': 1002},
            {'exe_name': 'firefox.exe', 'window_title': 'Reddit - Mozilla Firefox', 'pid': 1002},
            
            # Edge  
            {'exe_name': 'msedge.exe', 'window_title': 'Microsoft Edge', 'pid': 1003},
            {'exe_name': 'msedge.exe', 'window_title': 'LinkedIn - Microsoft Edge', 'pid': 1003},
        ...
        with patch('lb3.monitors.base.publish_event', side_effect=collect_event):
            ...
            for i, scenario in enumerate(browser_scenarios):
                with patch.object(monitor, '_get_active_window_info', return_value=scenario):
                    monitor.run_monitor_cycle()
        ...
        exe_names_detected = set(json.loads(e.attrs_json)['exe_name'] for e in collected_events)
        expected_browsers = {'chrome.exe', 'firefox.exe', 'msedge.exe', 'brave.exe', 'opera.exe'}
