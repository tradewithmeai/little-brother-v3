...
class TestCrashRecoveryIntegration:
    ...
    def test_mid_write_crash_simulation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert len(temp_files_before) >= 1, "Should have at least one temp file after writes"
            ...
            assert len(temp_files_after) == 0, "No temp files should remain after recovery"
    ...
    def test_corrupted_temp_file_recovery(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert recovery_report.temp_files_found == 1
            assert recovery_report.temp_files_recovered == 1
            assert recovery_report.total_lines_salvaged >= 3  # Should salvage valid lines
    ...
    def test_recovery_preserves_idempotency(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            stats = importer.flush_all_monitors(spool_dir)
            ...
            recovery_report2 = recover_all_temp_files(spool_dir)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for monitor in monitors:
                ...
                if monitor == "active_window":
                    # Valid temp file
                    temp_file = monitor_dir / "20250909-10.part"
                    temp_file.write_text('{"id": "aw1", "monitor": "active_window"}\n', encoding='utf-8')
            ...
            assert all(monitor in recovery_report.monitors_processed for monitor in monitors)
            ...
            for monitor in monitors:
                ...
                assert len(journal_files) >= 1, f"Monitor {monitor} should have recovered file"
...
"""
            ...
            events = [
                {"id": "gzip-event-1", "ts_utc": 123456789, "monitor": "test_monitor", "data": "test1"},
                {"id": "gzip-event-2", "ts_utc": 123456790, "monitor": "test_monitor", "data": "test2"},
                {"id": "gzip-event-3", "ts_utc": 123456791, "monitor": "test_monitor", "data": "test3"}
            ]
            ...
            assert "Recovered" in output and ("temp segments" in output or "temp files" in output)
            ...
            if not temp_file.exists() and monitor_dir.glob("*_recovered.ndjson.gz"):
                ...
                print(f"Recovery may not have worked - temp file exists: {temp_file.exists()}")
            ...
            if db_path.exists():
                db = Database(db_path)
                try:
                    cursor = db._connection.execute("SELECT COUNT(*) FROM events WHERE monitor = 'test_monitor'")
                    count = cursor.fetchone()[0]
                    assert count >= 1, "At least some events should have been imported after recovery"
                    ...
                    cursor2 = db._connection.execute("SELECT COUNT(*) FROM events WHERE monitor = 'test_monitor'")
                    final_count = cursor2.fetchone()[0]
                    assert final_count == count, "Should not create duplicates on second flush"
...
"""
            ...
            monitor_dir.mkdir()
            temp_file = monitor_dir / "20250909-10.part"
            temp_file.write_text('{"id": "test1", "ts_utc": 123456, "monitor": "test_monitor"}\n', encoding='utf-8')
            ...
            if db_path.exists():
                db = Database(db_path)
                try:
                    cursor = db._connection.execute("SELECT COUNT(*) FROM events WHERE id = 'test1'")
                    count = cursor.fetchone()[0]
    ...
    def test_concurrent_recovery_safety(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i in range(3):
                ...
                temp_file.write_text(f'{{"id": "event{i}", "monitor": "monitor{i}"}}\n', encoding='utf-8')
