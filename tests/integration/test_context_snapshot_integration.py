...
@skipUnless(
    sys.platform == "win32", "Context snapshot integration tests only run on Windows"
)
class TestContextSnapshotIntegration:
    ...
    def test_snapshot_on_foreground_change(self):
        ...
        assert (
            len(snapshots) >= 1
        ), f"Expected at least 1 snapshot, got {len(snapshots)}"
    ...
    def test_activity_counter_accuracy(self):
        ...
        assert (
            attrs["mouse_moves"] == 5
        ), f"Expected 5 mouse_moves, got {attrs['mouse_moves']}"
        assert (
            attrs["mouse_clicks"] == 4
        ), f"Expected 4 mouse_clicks, got {attrs['mouse_clicks']}"
        assert (
            attrs["mouse_scroll"] == 1
        ), f"Expected 1 mouse_scroll, got {attrs['mouse_scroll']}"
    ...
    def test_deduplication_in_gap_window(self):
        ...
        assert (
            additional_snapshots == 0
        ), f"Expected no additional snapshots, got {additional_snapshots}"
