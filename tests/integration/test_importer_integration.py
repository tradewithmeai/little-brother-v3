    ...
    def test_generate_and_import_10k_events_benchmark(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                events_per_minute = (stats["total_events_imported"] / import_time) * 60.0
                ...
                assert events_per_minute >= 5000, f"Performance below target: {events_per_minute:.0f} events/min (target: ≥5000)"
    ...
    def test_idempotent_import_repeat_benchmark(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                assert stats2["total_events_imported"] == 0, "Should not import any new events on second run"
                assert stats2["total_duplicates_skipped"] == event_count, "Should skip all duplicates"
                
                # Verify database count unchanged
                final_count = db.get_table_counts()["events"]
                assert final_count == initial_count, "Database count should remain stable"
                
                print("\nIdempotency test results:")
                print(f"First run: {stats1['total_events_imported']} imported, {stats1['total_duplicates_skipped']} skipped")
                print(f"Second run: {stats2['total_events_imported']} imported, {stats2['total_duplicates_skipped']} skipped")
    ...
    def test_error_handling_with_valid_files(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with gzip.open(corrupt_file, 'wt', encoding='utf-8') as f:
                ...
                corrupt_event = {"id": "corrupt-id", "ts_utc": 1234567890}  # Missing required fields
            ...
            try:
                ...
                assert stats["total_events_imported"] == valid_count  # Only valid events imported
                assert stats["total_files_with_errors"] == 1  # One corrupt file
                assert stats["total_files_processed"] >= 1  # At least the valid files processed
                
                # Check error sidecar was created
                error_file = corrupt_file.with_suffix(corrupt_file.suffix + '.error')
                assert error_file.exists(), "Error sidecar should be created for corrupt file"
                
                with open(error_file) as f:
                    error_data = json.load(f)
                    assert "error_message" in error_data
                    assert "first_failing_line" in error_data
                    assert error_data["first_failing_line"] == 2  # Second line was corrupt
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                for monitor, expected_count in monitor_configs.items():
                    ...
                    assert monitor_stats["events_imported"] == expected_count, f"Wrong count for {monitor}"
                
                # Verify _done directories created for all monitors
                done_dir = spool_dir / "_done"
                for monitor in monitor_configs:
                    monitor_done_dir = done_dir / monitor
                    assert monitor_done_dir.exists(), f"_done directory not created for {monitor}"
                    done_files = list(monitor_done_dir.glob("*.ndjson.gz"))
                    assert len(done_files) > 0, f"No files moved to _done for {monitor}"
                ...
                assert events_per_minute >= 3000, f"Mixed import performance below minimum: {events_per_minute:.0f} events/min"
