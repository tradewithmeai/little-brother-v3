...
"""
            ...
            try:
                ...
                try:
                    ...
                    assert (
                        return_code == 0
                    ), f"Process should exit cleanly, got code {return_code}"

                    # Check output for expected messages
                    output_lines = stdout.split("\n")
                    assert any(
                        "Starting monitoring system..." in line for line in output_lines
                    ), "Should show startup message"
                    assert any(
                        "Shutting down gracefully..." in line
                        or "graceful shutdown" in line.lower()
                    ...
                    if critical_errors:
                        print(
                            f"Warning: Found potential errors in stderr: {critical_errors[:3]}"
                        )
                    ...
                    assert (
                        len(monitor_dirs) > 0
                    ), "Should have created monitor directories in spool"
                    ...
                    for monitor_dir in monitor_dirs:
                        ...
                        print(
                            f"  {monitor_dir.name}: {len(journal_files)} journal files"
                        )

                    assert (
                        total_files > 0
                    ), "Should have created journal files during 10s run"
                    ...
                    assert (
                        flush_process.returncode == 0
                    ), f"Spool flush should succeed: {flush_process.stderr}"
                    ...
                    if db_path.exists():
                        ...
                        try:
                            ...
                            assert (
                                events_count > 0
                            ), "Database should contain events after flush"
                        finally:
                            db.close()
                    else:
                    ...
                    if status_process.returncode == 0:
                        ...
                        print(
                            ...
                            status_output[:200] + "..."
                            if len(status_output) > 200
                            else status_output,
                        )
                        assert (
                            "Monitor status:" in status_output
                        ), "Status should show monitor information"
                    else:
...
"""
            ...
            try:
                ...
                assert (
                    "Events will be printed to console" in stdout
                ), "Should indicate console output mode"

                # Should print event data to stdout
                lines = stdout.split("\n")
                event_lines = [
                    line for line in lines if "ts_utc" in line or '"id"' in line
                ]
                assert (
                    len(event_lines) > 0
                ), f"Should print events to console. Output: {stdout[:500]}"
                ...
                total_files = sum(
                    len(list(d.glob("*.ndjson.gz"))) for d in monitor_dirs
                )
                assert (
                    total_files == 0
                ), f"Dry-run should not create files, found {total_files} files"
...
"""
            ...
            assert (
                "Little Brother v3" in version_result.stdout
            ...
            if status_result.returncode == 0:
                ...
                print(
                    f"Status failed (expected for empty DB): {status_result.stderr[:100]}..."
                )
