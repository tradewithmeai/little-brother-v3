...
class TestULIDGeneration:
    ...
    def test_ulid_ordering_rapid_generation(self):
        ...
        assert (
            ulids == sorted_ulids
        ), "ULIDs should be monotonic even within same millisecond"
    ...
    def test_thread_safety_ordering(self):
        ...
        assert (
            sequential_ulids == sorted_sequential
        ), "Sequential ULIDs should be time-ordered"
        ...
        def generate_ulids_with_delays():
            thread_ulids = []
            for _i in range(ulids_per_thread):
                ulid = new_id()
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [
                executor.submit(generate_ulids_with_delays) for _ in range(num_threads)
            ]
        ...
        assert len(set(results)) == len(
            results
        ), "All concurrent ULIDs should be unique"
        for ulid in results:
            assert is_valid_id(ulid), f"ULID {ulid} should be valid"
    ...
    def test_high_concurrency_no_collisions(self):
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [
                executor.submit(generate_and_collect) for _ in range(num_threads)
            ]
        ...
        assert (
            len(all_ulids) == expected_total
        ), f"Should have {expected_total} unique ULIDs"
