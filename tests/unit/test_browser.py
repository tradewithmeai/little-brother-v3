...
class TestBrowserMonitor:
    ...
    def test_fallback_window_detection_non_browser(self, manual_scheduler):
        ...
        with patch("lb3.monitors.base.publish_event", side_effect=collect_event):
            with patch.object(
                monitor, "_get_active_window_info", return_value=mock_window_info
            ):
                monitor.start()
    ...
    def test_fallback_window_change_detection(self, fake_clock, manual_scheduler):
        ...
        with patch("lb3.monitors.base.publish_event", side_effect=collect_event):
            ...
            with patch.object(
                monitor, "_get_active_window_info", return_value=window_sequence[0]
            ):
                monitor.run_monitor_cycle()
            ...
            with patch.object(
                monitor, "_get_active_window_info", return_value=window_sequence[1]
            ):
                monitor.run_monitor_cycle()
            fake_clock.advance(1.0)
            ...
            with patch.object(
                monitor, "_get_active_window_info", return_value=window_sequence[2]
            ):
                monitor.run_monitor_cycle()
    ...
    def test_window_title_hashing_invoked(self, mock_hash_str, manual_scheduler):
        ...
        hash_calls = [
            call
            for call in mock_hash_str.call_args_list
