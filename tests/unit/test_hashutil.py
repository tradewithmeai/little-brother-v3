...
class TestPurposeScopedHashing:
    ...
    def test_purpose_scoping(self):
        ...
        assert len(set(hash_values)) == len(hash_values), "All purpose hashes should be unique"
    ...
    def test_salt_effect(self):
        ...
        different_config = Config(
            hashing=HashingConfig(salt="aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        )
        
        with patch('lb3.hashutil.get_effective_config', return_value=different_config):
            different_salt_hash = hash_str(value, purpose)
        
        assert original_hash != different_salt_hash, "Different salts should produce different hashes"
    ...
    def test_hash_formula_structure(self):
        ...
        assert actual_hash == expected_hash, "Hash should follow salt||purpose||0x00||utf8(value) formula"
    ...
    def test_concurrent_hashing(self):
        ...
        for hash_result in all_hashes:
            assert hash_result == expected_hash, "Concurrent hashing should be consistent"
...
class TestHashConsistency:
    ...
    def test_cross_platform_consistency(self):
        ...
        for value, purpose in test_cases:
            ...
            assert hash1 == hash2, f"Hash should be consistent for {value!r} with {purpose}"
    ...
    def test_salt_isolation(self):
        ...
        config1 = Config(
            hashing=HashingConfig(salt="1111111111111111111111111111111111111111111111111111111111111111")
        )
        config2 = Config(
            hashing=HashingConfig(salt="2222222222222222222222222222222222222222222222222222222222222222")
        )
