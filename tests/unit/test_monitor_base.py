...
class TestMonitorBase:
    ...
    def test_emit_event_validation(self, mock_config):
        ...
        # Valid event
        valid_event = {"action": "test_action", "subject_type": "test_subject"}

        # Should not raise exception
        monitor.emit(valid_event)

        # Invalid event - missing action
        invalid_event = {"subject_type": "test_subject"}
    ...
    def test_thread_safety(self, mock_config):
        ...
        def emit_events(start_idx, count):
            for i in range(count):
                monitor.emit(
                    {"action": f"test_{start_idx + i}", "subject_type": "test"}
                )

        # Create multiple threads
        threads = []
        for i in range(3):
            thread = threading.Thread(target=emit_events, args=(i * 10, 5))
