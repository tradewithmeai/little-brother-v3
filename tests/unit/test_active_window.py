...
class TestActiveWindowMonitor:
    ...
    def test_get_current_window_info(
        self,
        mock_dword_class,
        mock_byref,
        mock_new_id,
    ...
    @patch("lb3.monitors.active_window.get_database")
    @patch("lb3.monitors.active_window.GetForegroundWindow")
    def test_get_current_window_info_no_window(
        self, mock_get_foreground, mock_get_database
    ):
    ...
    def test_get_or_create_app_id_new(self, mock_new_id, mock_get_database):
        ...
        mock_db.execute_query.assert_called_once_with(
            "SELECT id FROM apps WHERE exe_path_hash = ?", ("test_hash",)
        )
    ...
    def test_hashing_is_invoked(self, mock_hash_str, mock_get_database):
        ...
        mock_hash_str.side_effect = (
            lambda text, purpose: f"hashed_{purpose}_{len(text)}"
        )

        # Mock other dependencies
        with (
            patch("lb3.monitors.active_window.GetForegroundWindow", return_value=123),
            ...
            patch(
                "lb3.monitors.active_window.GetWindowThreadProcessId"
            ) as mock_get_pid,
            patch("lb3.monitors.active_window.psutil.Process") as mock_process,
            patch(
                "lb3.monitors.active_window.new_id", side_effect=["app_id", "window_id"]
            ),
            ...
            assert (
                window_info.exe_path_hash == "hashed_exe_path_17"
            )  # len("/path/to/test.exe")
