...
class TestKeyboardMonitor:
    ...
    def test_guardrail_enforcement_at_init(self, mock_config_disabled_guardrails):
        ...
        with (
            patch(
                "lb3.monitors.keyboard.get_effective_config",
                return_value=mock_config_disabled_guardrails,
            ),
            pytest.raises(GuardrailViolationError),
        ):
            KeyboardMonitor(dry_run=True)

    def test_initialization_with_guardrails(
    ...
    def test_deterministic_timing_with_fake_clock(
        self, mock_config, fake_clock, manual_scheduler
    ):
    ...
    def test_size_based_flush_at_128_events(self, mock_config, manual_scheduler):
        ...
        batch_config = BatchConfig(
            max_size=128, max_time_s=999.0
        )  # High time so only size matters
    ...
    def test_time_based_flush_deterministic(
        self, mock_config, fake_clock, manual_scheduler
    ):
    ...
    def test_event_source_auto_selection(self, mock_config, manual_scheduler):
        ...
        with patch(
            ...
            monitor = KeyboardMonitor(dry_run=True, scheduler=manual_scheduler)
            assert isinstance(monitor._event_source, FakeKeyboardSource)
